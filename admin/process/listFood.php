...
$query = "SELECT food_id FROM food,food_category,foodsearch WHERE food_category.foodcat_id = food.foodcat_id AND food_category.foodcat_name = foodsearch.cat_name";
...
if($pn == 1){
    ...
    $centerPages.= "<a href='listFood.php?pn = $add1'>$add1</a>";
    
}else if($pn == $lastPage){
    $centerPages.="<a href='listFood.php?pn=$sub1'>$sub1</a>";
    $centerPages.="<span class = 'pageNumActive'>$pn</span>";
}else if($pn > 2 && $pn < ($lastPage-1)){
    $centerPages.="<a href='listFood.php?pn=$sub2'>$sub2</a>";
    $centerPages.="<a href='listFood.php?pn=$sub1'>$sub1</a>";
    $centerPages.="<span class = 'pageNumActive'>$pn</span>";
    $centerPages.="<a href='listFood.php?pn=$add1'>$add1</a>";
    $centerPages.="<a href='listFood.php?pn=$add2'>$add2</a>";
...
$query1 ="SELECT food_category.foodcat_name,food.food_id,food.food_name,food.price FROM food,food_category,foodsearch WHERE food_category.foodcat_id = food.foodcat_id AND food_category.foodcat_name = foodsearch.cat_name $limit";
$result1 = $database->execute($query1);
	...
	while ($listUser = $database-> fetch_record($result1)) {
                                        ...
                                        echo "<td><a class = 'btn btn-primary' id = 'edit' href ='../process/editFood.php?food_id=$_SESSION[count]'>EDIT</a></td>";
                                        echo "<td><a class = 'btn btn-primary' id = 'edit' href ='../process/deleteFood.php?food_id=$_SESSION[count]'>DELETE</a></td>";
...
if($lastPage != 1){
    ...
    if($pn != 1){
    $previous = $pn - 1;
    $paginationDisplay .= "<a href='listFood.php?pn=$previous'><button class='' >Back</button></a>";
    }
    $paginationDisplay .= "<span class = 'pageNumbers'>$centerPages</span>";
    if($pn != $lastPage){
    $nextPage = $pn +1;
    $paginationDisplay .= "<a href='listFood.php?pn=$nextPage'><button class='' >Next</button></a>";
    }
}
